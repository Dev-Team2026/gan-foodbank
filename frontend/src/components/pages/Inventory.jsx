...
const Inventory = () => {
    ...
    const handleOnSubmitStringValueEdit = async (newValue, targetValue) => {
        ...
        try {
            await axios.patch(`http://localhost:3000/inventoryStringValue`, {
                item: item,
                newValue: newValue,
    ...
    return (
        <div className="container">
            ...
            />}
            {currentAction[0] === "editStock" &&
                <EditItemIntForm item={inventory.find(item => item.item_id === currentAction[1])}
                                 inventoryItemForm={inventoryItemForm} currentAction={currentAction}
                                 handleOnChangeItemForm={handleOnChangeItemForm}
                                 handleOnSubmitStockEdit={handleOnSubmitStockEdit}/>}
            {currentAction[0] === "editName" &&
                <EditItemStringForm value={"name"} inventoryItemFormValue={inventoryItemForm.name}
                                    handleOnChangeItemForm={handleOnChangeItemForm}
                                    handleOnSubmitStringValueEdit={handleOnSubmitStringValueEdit}/>}
            {currentAction[0] === "editCategory" &&
                <EditItemStringForm value={"category"} inventoryItemFormValue={inventoryItemForm.category}
                                    handleOnChangeItemForm={handleOnChangeItemForm}
                                    handleOnSubmitStringValueEdit={handleOnSubmitStringValueEdit}/>}
            ...
            <table>
                ...
                <tr>
                    <th>
                        ...
                        <button className="tableBtn" onClick={() => sort("nameDesc")}>Desc</button>
                        <br/>
                        Item <br/>
                        {userData?.role === 1 &&
                            <button className="tableBtn" onClick={() => setupAction("selectItemNameToEdit")}>Update Item
                                Name</button>}
                    </th>
                    <th>
                        <button className="tableBtn" onClick={() => sort("categoryAsc")}>Asc</button>
                        <button className="tableBtn" onClick={() => sort("categoryDesc")}>Desc</button>
                        <br/>
                        Category <br/>
                        {userData?.role === 1 &&
                            <button className="tableBtn" onClick={() => setupAction("selectItemCategoryToEdit")}>Update
                                Item Category</button>}
                    </th>
                    <th>
                        <button className="tableBtn" onClick={() => sort("stockAsc")}>Asc</button>
                        <button className="tableBtn" onClick={() => sort("stockDesc")}>Desc</button>
                        <br/>
                        Count <br/>
                        {userData?.role === 1 &&
                            <button className="tableBtn" onClick={() => setupAction("selectStockToEdit")}>Update Item
                                Stock</button>}
                    </th>
                </tr>
                </thead>
                <tbody>
                {inventory.map((item) => (
                    <InventoryCard key={item.item_id} {...item} currentAction={currentAction}
                                   handleOnSelect={handleOnSelect} setUpForEditing={setUpForEditing}
                                   handleAddToNewOrder={handleAddToNewOrder}
        ...
        Page {pagination.page} of {pagination.totalPages}
            ...
            {currentAction === "order" &&
                <NewOrderCard orderItems={newOrder} handleSubmitNewOrder={handleSubmitNewOrder}
                              handleRemoveItemFromOrder={handleRemoveItemFromOrder}
                              handleAdjustUnitQuatity={handleAdjustUnitQuatity} setIsDonation={setIsDonation}/>}
        </div>
    )
}
